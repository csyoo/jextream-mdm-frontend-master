    ...
    <v-dialog persistent v-model="manageFirmwareDialog" max-width="600px">
      <v-card class="cardBg">
        <v-card-title class="medium cardTitle">
          <span>Manage Firmware - Check and Update</span>
          <v-spacer></v-spacer>
...
<script>
import { router_group_firmware_update } from "@/graphql/mutations.js";
  ...
  watch: {
    manageFirmwareDialog: {
      handler() {
        ...
        this.viewRouterName = this.firmwareRow.group_name;
        this.manageFirmDate = this.firmwareResponse.model_config_added_on;
        this.manageFirmModel = this.firmwareResponse.model_no;
        this.firmwareId = this.firmwareRow.router_group_id;
      },
      immediate: true,
    },
  },
      ...
      if (this.schdeduleDate == null || this.time == null) {
        ...
        try {
          let result = await API.graphql(
            graphqlOperation(router_group_firmware_update, {
              input: {
                router_group_id: this.firmwareId,
                schedule_date: data,
              },
            })
          );
