<template>
  <div>
    <!-- dialog for adding the apn  -->
    <v-dialog v-model="apnDialogAdd" persistent max-width="600px">
      <v-card flat class="cardBg dialogCard">
        <v-toolbar dense class="dialogToolbar cardBg">
          <v-toolbar-title> Add > New APN</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-btn icon @click="apn_clear_data()">
        ...
        <v-card-actions class="button-corner">
          <v-spacer></v-spacer>
          <v-btn class="button tabledata--text mt-2" @click="apn_clear_data()"
            >Cancel</v-btn
          >
          <v-btn
...
export default {
  ...
  methods: {
    //method to set the input limit for characters
    set_limit(value) {
    ...
    //method to validate the newly added record
    validate_add_data() {
      ...
      for (var i = 0; i < this.apnData.length; i++) {
        if (this.apnName.trim() == this.apnData[i].ProfileName) {
          valid = false;
          let Toggle = 15;
          this.$emit("clicked", 15);
        }
      }
      if (
        this.apnName.trim() == "" ||
      ...
      } else if (this.apnData.length > 15) {
        var valid = false;
        let Toggle = 6;
        this.$emit("clicked", 6);
      }
    ...
    //method to save the newly added record
    store_apn_values() {
      var data = {
        Index: this.apnData.length + 1,
      ...
      this.$emit("apnAddData", data);
      this.$emit("clicked", 3);
      this.apn_clear_data();
    },

    //method invokes on close action for clearing the fields
    apn_clear_data() {
      this.$emit("clicked", 1);
      this.apnName = "";
      this.carrierName = "";
